...
sed -i s#pasv_addr_resolve=.*#pasv_addr_resolve=${PASV_ADDR_RESOLVE}#g /etc/vsftpd/vsftpd.conf
sed -i s#pasv_enable=.*#pasv_enable=${PASV_ENABLE}#g /etc/vsftpd/vsftpd.conf
sed -i s#file_open_mode=.*#file_open_mode=${FILE_OPEN_MODE}#g /etc/vsftpd/vsftpd.conf
sed -i s#local_umask=.*#local_umask=${LOCAL_UMASK}#g /etc/vsftpd/vsftpd.conf
sed -i s#xferlog_std_format=.*#xferlog_std_format=${XFERLOG_STD_FORMAT}#g /etc/vsftpd/vsftpd.conf
sed -i s#reverse_lookup_enable=.*#reverse_lookup_enable=${REVERSE_LOOKUP_ENABLE}#g /etc/vsftpd/vsftpd.conf
