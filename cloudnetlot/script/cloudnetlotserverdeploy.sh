...
if [ "$2" = "true" ];then
    docker run --name cloudnetlotserver -p 9090:80 -v ${BASE}/cloudnetlotserver/nginx:/etc/nginx -v ${BASE}/cloudnetlotserver/php:/etc/php/5.6 -v ${BASE}/www:/usr/local/www -v /etc/localtime:/etc/localtime:ro --restart=always -it -d ${imagename}
else
    docker run --name cloudnetlotserver -p 9090:80 -v ${BASE}/www:/usr/local/www -v /etc/localtime:/etc/localtime:ro --restart=always -it -d ${imagename}
fi
